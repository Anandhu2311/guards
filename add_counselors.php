...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    $counselor_email = trim($_POST['counselor_email']);
    $role_id = 3; // Set role_id for counselor

    // Check if email already exists
    $stmt = $pdo->prepare("SELECT coun_email FROM counselors WHERE coun_email = ?");
    $stmt->execute([$counselor_email]);
    if ($stmt->rowCount() > 0) {
        $message = "<div class='alert-error'>Error: This email is already registered as a counselor.</div>";
    } else {
        ...
        $raw_password = generateSecurePassword(8); // Generate a secure password of length 8
        $hashed_password = password_hash($raw_password, PASSWORD_DEFAULT); // Hash the password for storage

        // Insert into counselors table with role_id
        $stmt = $pdo->prepare("INSERT INTO counselors (coun_email, coun_password, password_updated, role_id) VALUES (?, ?, ?, ?)");
        if ($stmt->execute([$counselor_email, $hashed_password, false, $role_id])) {
            ...
            try {
                ...
                $mail->addAddress($counselor_email);
                $mail->Subject = 'Your GuardSphere Counselor Account';
                $mail->Body = "Hello,\n\nAn admin has created your counselor account.\n\nEmail: $counselor_email\nPassword: $raw_password\n\nPlease log in and change your password.";

                if ($mail->send()) {
                    $message = "<div class='alert-success'>Counselor added successfully! Email sent.</div>";
                } else {
                    $message = "<div class='alert-warning'>Counselor added, but email sending failed.</div>";
                }
            } catch (Exception $e) {
                $message = "<div class='alert-warning'>Counselor added, but email could not be sent. Error: " . $mail->ErrorInfo . "</div>";
            }
        } else {
            $message = "<div class='alert-error'>Error adding counselor.</div>";
        }
    }
}
?>
...
<head>
    ...
    <title>Add Counselor - GuardSphere Admin</title>
...
<body>
    ...
    <div class="sidebar" id="sidebar">
        ...
        <div class="sidebar-item">
            ...
            <div class="sub-options" style="display: block;">
                <a href="add_counselors.php" class="sub-option active">
    ...
    <div class="main-content" id="mainContent">
        <div class="form-card">
            <h2><i class="fas fa-user-plus"></i> Add New Counselor</h2>
            ...
            <form method="POST" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>">
                <div class="form-group">
                    ...
                    <input type="email" id="counselor_email" name="counselor_email" class="form-control" required 
                           placeholder="Enter counselor's email address">
