...
ini_set('error_log', 'advisor_dashboard.log');
...
// Debug log to check advisor email
error_log("Advisor email from session: " . $advisor_email);

// Get advisor's bookings
$advisorBookings = getBookings($pdo, $advisor_email, 'provider');

// Debug log to check number of bookings
error_log("Number of bookings found: " . count($advisorBookings));
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        error_log("Attempting availability update - Email: $advisor_email, Schedule: $scheduleId, Available: $isAvailable");
        echo simpleAvailabilityUpdate($pdo, $advisor_email, $scheduleId, $isAvailable);
        exit();
    } catch (Exception $e) {
...
ini_set('display_errors', 1);
error_reporting(E_ALL);
error_log("=== ADV_DASHBOARD.PHP START ===");
error_log("Session ID: " . session_id());
error_log("Session Data: " . print_r($_SESSION, true));
error_log("Request Method: " . $_SERVER['REQUEST_METHOD']);
error_log("=== ADV_DASHBOARD.PHP END ===");
...
// Get advisor email from session BEFORE using it
$advisor_email = $_SESSION['email'];
...
try {
    ...
    if (!$advisor) {
        error_log("Advisor not found or inactive: " . $advisor_email);
        session_destroy();
    ...
    $_SESSION['user_id'] = $advisor['adv_id'];
    $_SESSION['adv_id'] = $advisor['adv_id'];
    $_SESSION['email'] = $advisor['adv_email'];
    $_SESSION['user_email'] = $advisor['adv_email'];
    $_SESSION['role_id'] = 2;
...
function simpleAvailabilityUpdate($conn, $email, $scheduleId, $isAvailable) {
    try {
        // Get advisor name
        $stmt = $conn->prepare("SELECT adv_name as staff_name FROM advisors WHERE adv_email = ? AND is_active = 1");
        $stmt->execute([$email]);
...
$userAvailability = getAdvisorAvailability($pdo, $advisor_email);

// Get advisor's bookings with proper role
$advisorBookings = getBookings($pdo, $advisor_email, 'provider');

function getAdvisorSchedules($pdo) {
    try {
        ...
        error_log('Error getting advisor schedules: ' . $e->getMessage());
        return [];
    }
}

function getAdvisorAvailability($pdo, $email) {
    try {
...
<head>
    ...
    <title>Advisor Dashboard - GuardSphere</title>
...
<body>
    <div class="container">
            ...
            <nav>
                ...
                <div class="profile-section">
                    <div class="user-avatar" onclick="toggleDropdown()">
                        <?php 
                            if (isset($advisor['adv_name']) && !empty($advisor['adv_name'])) {
                                echo htmlspecialchars(substr($advisor['adv_name'], 0, 1));
                            } else {
                                echo htmlspecialchars(substr($advisor['adv_email'], 0, 1));
                            }
                        ?>
                    </div>
        ...
        <div class="sidebar">
            <div class="sidebar-content">
                <div class="welcome-text">Welcome Advisor</div>
                <div class="user-email"><?php echo htmlspecialchars($advisor['adv_email']); ?></div>
        ...
        <div class="main-content">
            <div id="dashboardOverview" class="content-section">
                ...
                <div class="stats-grid">
                    ...
                    <div class="stat-card">
                        ...
                        <div class="stat-value"><?php echo count(array_filter($advisorBookings, function($booking) { 
            ...
            <div id="bookingsSection" class="content-section">
                ...
                <div class="bookings-table-container">
                    <?php
                    // Debug information
                    echo "<!-- Debug Info: Advisor Email = " . htmlspecialchars($advisor_email) . " -->";
                    echo "<!-- Debug Info: Number of Bookings = " . count($advisorBookings) . " -->";
                    
                    if (empty($advisorBookings)): 
                    ?>
                        <div style="text-align: center; padding: 20px;">
                            <p>No bookings found for email: <?php echo htmlspecialchars($advisor_email); ?></p>
                        </div>
                    <?php else: ?>
            ...
            <div id="profileSection" class="content-section" style="display: none;">
                <h1 class="section-title">Advisor Profile</h1>
                ...
                <div id="viewProfile" class="profile-container">
                    <div class="profile-info">
                        <div class="info-group">
                            ...
                            <p><?php echo htmlspecialchars($advisor['adv_name'] ?? 'Not set'); ?></p>
                        </div>
                        
                        <div class="info-group">
                            <label>Email</label>
                            <p><?php echo htmlspecialchars($advisor['adv_email']); ?></p>
                        </div>
                        
                        <div class="info-group">
                            <label>Phone Number</label>
                            <p><?php echo htmlspecialchars($advisor['phone_number'] ?? 'Not set'); ?></p>
                        </div>
                        
                        <div class="info-group">
                            <label>Location</label>
                            <p><?php echo htmlspecialchars($advisor['adv_location'] ?? 'Not set'); ?></p>
                        </div>
                        
                        <div class="info-group">
                            <label>Areas of Expertise</label>
                            <p><?php echo htmlspecialchars($advisor['adv_specialization'] ?? 'Not set'); ?></p>
                        </div>
                    </div>
                ...
                <div id="editProfile" class="profile-form-container" style="display: none;">
                    <form id="advisorProfileForm" method="POST" action="update_advisor_profile.php">
                        <div class="form-group">
                            <label for="name">Full Name</label>
                            <input type="text" 
                                   id="name" 
                                   name="name" 
                                   value="<?php echo htmlspecialchars($advisor['adv_name'] ?? ''); ?>" 
                        ...
                        <div class="form-group">
                            ...
                            <input type="tel" 
                                   id="phone" 
                                   name="phone" 
                                   value="<?php echo htmlspecialchars($advisor['phone_number'] ?? ''); ?>" 
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" 
                                   id="location" 
                                   name="location" 
                                   value="<?php echo htmlspecialchars($advisor['adv_location'] ?? ''); ?>" 
                        ...
                        <div class="form-group">
                            ...
                            <textarea id="expertise" 
                                      ...
                                      required><?php echo htmlspecialchars($advisor['adv_specialization'] ?? ''); ?></textarea>
                        </div>
    ...
    <script>
        ...
        function toggleAvailability(scheduleId, currentStatus) {
            ...
            // Send request to update_availability_adv.php instead of update_availability.php
            fetch('update_availability_adv.php', {
                method: 'POST',
                body: formData,
        ...
        document.getElementById('advisorProfileForm').addEventListener('submit', function(e) {
            e.preventDefault();
        ...
        function showMedicalForm(patientName, patientEmail, bookingId) {
            ...
            fetch(`get_medical_notes_adv.php?booking_id=${bookingId}`)
                .then(response => response.json())
                .then(data => {
            ...
            form.addEventListener('submit', async (e) => {
                ...
                try {
                    // Save medical notes
                    const response = await fetch('update_medical_notes_adv.php', {
                        method: 'POST',
                        body: formData
                    });
        ...
        function updateBookingStatus(bookingId, status) {
            ...
            formData.append('provider_email', '<?php echo $advisor_email; ?>');
