...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (strlen($new_password) < 6) {
        ...
        $stmt = $pdo->prepare("UPDATE counselors SET coun_password = ?, password_updated = 1 WHERE coun_id = ?");
        if ($stmt->execute([$hashed_password, $_SESSION['coun_id']])) {
            $updateSuccess = "Password updated successfully! Please log in again.";
            session_destroy();
            header("Location: counselor_signin.php"); // Redirect to login page
            exit();
        } else {
