...
if (!isset($_SESSION['email'])) {
    header('Location: coun_forgot.php');
    exit();
}
$conn= new mysqli('localhost','root','','guardsphere');
if ($conn->connect_error) {
    ...
    if($_SERVER["REQUEST_METHOD"]=="POST"){
        ...
        if($password==$confirm_password){
            ...
            $sql = "UPDATE counselors SET coun_password = '$hashed_password' WHERE coun_email = '" . $_SESSION['email'] . "'";
           
            if ($conn->query($sql) === TRUE) {
                $_SESSION['success_message'] = "Your password has been successfully updated!";
                header('Location: counselor_signin.php');
...
<body>
    ...
    function checkPassword() {
        ...
        // Corrected regex pattern: ensure at least one lowercase, one uppercase, and one digit, with min 8 characters
