...
if (isset($_GET['delete']) && is_numeric($_GET['delete'])) {
    $advisor_id = $_GET['delete'];
    $delete_stmt = $pdo->prepare("DELETE FROM advisors WHERE id = ?");
    if ($delete_stmt->execute([$advisor_id])) {
        $message = "<div class='alert-success'>Advisor deleted successfully!</div>";
    } else {
        $message = "<div class='alert-error'>Failed to delete advisor.</div>";
    }
}

// Add a toggle status handler 
if (isset($_GET['toggle']) && is_numeric($_GET['toggle'])) {
    $advisor_id = $_GET['toggle'];
    
    // First get current status
    $status_query = $pdo->prepare("SELECT is_active, adv_email, adv_name FROM advisors WHERE adv_id = ?");
    $status_query->execute([$advisor_id]);
    ...
    $update_stmt = $pdo->prepare("UPDATE advisors SET is_active = ? WHERE adv_id = ?");
    if ($update_stmt->execute([$new_status, $advisor_id])) {
        // Send email notification
        $advisor_email = $current_status['adv_email'];
        $advisor_name = !empty($current_status['adv_name']) ? $current_status['adv_name'] : "Advisor";
        $status_text = $new_status ? "enabled" : "disabled";
        
        $subject = "GuardSphere Advisor Account " . ucfirst($status_text);
        $message_body = "Dear " . $advisor_name . ",\n\n";
        $message_body .= "Your advisor account has been " . $status_text . " by the administrator.\n";
        if ($new_status) {
            $message_body .= "You can now log in to your account and provide advisory services.\n";
        } else {
        ...
        if (sendEmail($advisor_email, $subject, $message_body)) {
            $message = "<div class='alert-success'>Advisor account " . $status_text . " successfully and email notification sent!</div>";
        } else {
            $message = "<div class='alert-warning'>Advisor account " . $status_text . " successfully but failed to send email notification. Please check server logs.</div>";
        }
    } else {
        $message = "<div class='alert-error'>Failed to update advisor status.</div>";
    }
}

// Fetch all advisors
$sql = "SELECT * FROM advisors ORDER BY adv_id DESC"; // Adjust the query as needed
...
<head>
    ...
    <title>Manage Advisors - GuardSphere Admin</title>
    ...
    <style>
        * {
        ...
        .advisor-list-header .add-btn:hover {
            background: #7a44b8;
...
<body>
    ...
    <div class="sidebar" id="sidebar">
        ...
        <div class="sidebar-item">
            ...
            <div class="sub-options" style="display: block;">
                ...
                <a href="manage_advisors.php" class="sub-option active">
                    <i class="fas fa-eye"></i> 
    ...
    <div class="main-content" id="mainContent">
        <div class="advisor-list-container">
            <div class="advisor-list-header">
                <h2><i class="fas fa-user-tie"></i> Manage Advisors</h2>
                <a href="add_advisor.php" class="add-btn">
                ...
                <div class="advisor-list">
                    <?php while ($row = $result->fetch(PDO::FETCH_ASSOC)): ?>
                        <div class="advisor-card">
                            <?php 
                            $status = isset($row['password_updated']) && $row['password_updated'] ? 'active' : 'pending';
                            $statusText = $status === 'active' ? 'Active' : 'Pending'; 
                            ?>
                            <span class="status-badge status-<?php echo $status; ?>"><?php echo $statusText; ?></span>
                            
                            <div class="advisor-info">
                                <div class="advisor-name">
                                    <?php echo !empty($row['adv_name']) ? htmlspecialchars($row['adv_name']) : 'Not Set'; ?>
                                </div>
                                <div class="advisor-detail">
                                    <i class="fas fa-envelope"></i>
                                    <?php echo htmlspecialchars($row['adv_email']); ?>
                                </div>
                                <div class="advisor-detail">
                                    <i class="fas fa-phone"></i>
                                    <?php echo !empty($row['adv_phone_number']) ? htmlspecialchars($row['adv_phone_number']) : 'Not Set'; ?>
                                </div>
                                <div class="advisor-detail">
                            ...
                            <div class="advisor-actions">
                                <?php if (isset($row['is_active']) && $row['is_active'] == 1): ?>
                                    <button class="action-btn disable-btn" onclick="confirmToggle(<?php echo $row['adv_id']; ?>, '<?php echo htmlspecialchars($row['adv_email']); ?>', 'disable')">
                                        <i class="fas fa-ban"></i> Disable
                                    </button>
                                <?php else: ?>
                                    <button class="action-btn enable-btn" onclick="confirmToggle(<?php echo $row['adv_id']; ?>, '<?php echo htmlspecialchars($row['adv_email']); ?>', 'enable')">
            ...
            <?php else: ?>
                <div class="empty-state">
                    ...
                    <p>There are no advisors in the system yet. Add your first advisor to get started.</p>
                    <a href="add_advisor.php" class="btn">
    ...
    <script>
        ...
        function confirmToggle(id, email, action) {
            if (confirm(`Are you sure you want to ${action} advisor with email: ${email}?`)) {
                window.location.href = `manage_advisors.php?toggle=${id}`;
            }
        }
