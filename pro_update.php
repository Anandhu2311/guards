...
// Update the database connection and fetch user details using PDO
$user_details = array();
if (isset($_SESSION['email'])) {
    try {
        ...
        $stmt = $pdo->prepare("SELECT name, phone_number FROM users WHERE email = ?");
        $stmt->execute([$email]);
...
// Update this section at the top of your PHP code
require_once 'fetch_emergency_contacts.php';
...
if (isset($_SESSION['email'])) {
    try {
        ...
        $stmt = $pdo->prepare("SELECT * FROM emergency_contacts WHERE email = ?");
        $stmt->execute([$_SESSION['email']]);
        $emergency_contact = $stmt->fetchAll(PDO::FETCH_ASSOC);
        
        // Add debug output
        if (empty($emergency_contact)) {
            error_log("No emergency contacts found for user: " . $_SESSION['email']);
        }
    } catch(PDOException $e) {
        ...
        <div class="profile-sidebar">
            <div class="profile-photo">
                ...
                if (isset($user_details['name']) && !empty($user_details['name'])) {
                    echo strtoupper(substr($user_details['name'], 0, 2));
                } else {
                    echo 'US';
                }
                ?>
            ...
            <div class="profile-name">
                <?php 
                echo isset($user_details['name']) ? htmlspecialchars($user_details['name']) : 'User Name';
                ?>
            </div>
            <div class="sidebar-buttons">
                <a href="edit_profile.php" class="sidebar-btn" style="text-decoration: none;"><i class="fas fa-user-edit"></i> Edit Profile</a>
                <a href="security_settings.php" class="sidebar-btn" style="text-decoration: none;"><i class="fas fa-shield-alt"></i> Security Settings</a>
                <a href="emergency_contacts.php" class="sidebar-btn" style="text-decoration: none;"><i class="fas fa-phone-alt"></i> Emergency Contacts</a>
                <a href="privacy_settings.php" class="sidebar-btn" style="text-decoration: none;"><i class="fas fa-lock"></i> Privacy Settings</a>
            </div>
        </div>
        ...
        <div class="profile-details">
            <div class="card">
                ...
                <div class="profile-info">
                    <div class="info-group">
                        <div class="info-item">
                            <label>Name</label>
                            <span class="info-value"><?php echo isset($user_details['name']) ? htmlspecialchars($user_details['name']) : 'Not set'; ?></span>
                        </div>
                        <div class="info-item">
                            <label>Phone Number</label>
                            <span class="info-value"><?php echo isset($user_details['phone_number']) ? htmlspecialchars($user_details['phone_number']) : 'Not set'; ?></span>
                        </div>
                    </div>
                    <div class="info-group">
                        <div class="info-item">
            ...
            <div class="card">
                ...
                <div class="contact-info">
                    ...
                    // Debug output
                    if (isset($_SESSION['email'])) {
                        echo "<!-- Debug: User email: " . htmlspecialchars($_SESSION['email']) . " -->";
                    }
                    echo "<!-- Debug: Number of contacts: " . count($emergency_contact) . " -->";
                    ?>
                        ...
                        <?php foreach ($emergency_contact as $contact): ?>
                            <div class="info-group">
                                <div class="info-item">
                                    ...
                                    <span class="info-value"><?php echo htmlspecialchars($contact['emergency_name'] ?? 'Not set'); ?></span>
                                </div>
                                <div class="info-item">
                                    <label>Email</label>
                                    <span class="info-value"><?php echo htmlspecialchars($contact['emergency_email'] ?? 'Not set'); ?></span>
                                </div>
                            </div>
                            <div class="info-group">
                                <div class="info-item">
                                    <label>Phone Number</label>
                                    <span class="info-value"><?php echo htmlspecialchars($contact['em_number'] ?? 'Not set'); ?></span>
                                </div>
                                <div class="info-item">
                                    <label>Relationship</label>
                                    <span class="info-value"><?php echo htmlspecialchars($contact['relationship'] ?? 'Not set'); ?></span>
                                </div>
                            </div>
                            <hr style="border: 1px solid #333; margin: 1rem 0;">
                        <?php endforeach; ?>
                    <?php else: ?>
                        <p style="color: #cccccc; text-align: center;">No emergency contacts added yet.</p>
                    <?php endif; ?>
                </div>
            </div>
