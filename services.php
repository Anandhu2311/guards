            ...
            if (confirm(confirmMsg)) {
                ...
                .then(data => {
                    ...
                    setTimeout(() => {
                        ...
                        statusAlert.style.transition = 'opacity 1s ease';
                        setTimeout(() => {
    ...
    <script>
        ...
        function loadAvailableSlots(serviceType = '') {
            ...
            .then(data => {
                ...
                // Filter for only available slots (where is_available === 1)
                const availableSlots = data.schedules.filter(slot => slot.is_available === 1);
        ...
        // Make sure tab switching properly calls loadMyBookings
        document.addEventListener('DOMContentLoaded', function() {
            // Initialize the tab system
            const tabButtons = document.querySelectorAll('.tab-btn');
        ...
        // Fix the displayBookings function
        function displayBookings(bookings, container) {
            showDebugMessage("Displaying bookings...");
            let html = '<div class="bookings-container">';
            ...
            const activeBookings = bookings.filter(booking => 
                ...
                booking.status !== 'follow_up_completed'
            );
            ...
            if (activeBookings.length === 0) {
                ...
                activeBookings.forEach(booking => {
                    ...
                    <div class="booking-card ${statusClass}">
                        <div class="booking-header">
                            <h3>${booking.service_type_name || booking.service_type || 'Service Booking'}</h3>
                            <span class="booking-status ${booking.status}">${booking.status}</span>
                        </div>
                        <div class="booking-details">
                            <p><strong>Booked on:</strong> ${formattedDate}</p>
                            <p><strong>Day:</strong> ${booking.day || 'N/A'}</p>
                            <p><strong>Time:</strong> ${booking.start_time || ''} - ${booking.end_time || ''}</p>
                            <p><strong>Provider:</strong> ${booking.provider_name || 'Staff'}</p>
                            <p><strong>Location:</strong> ${booking.location || 'Online'}</p>
                        </div>
                        ${booking.status === 'pending' ? `
                        <div class="booking-actions">
            ...
            // Add event listener to "Book Now" button if it exists
            const bookNowBtn = container.querySelector('#bookNowBtn');
            if (bookNowBtn) {
                bookNowBtn.addEventListener('click', function() {
                    const servicesTabBtn = document.querySelector('.tab-btn[data-tab="services"]');
                    if (servicesTabBtn) {
