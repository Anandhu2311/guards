...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (strlen($new_password) < 6) {
        ...
        $stmt = $pdo->prepare("UPDATE supporters SET sup_password = ?, password_updated = 1 WHERE sup_id = ?");
        if ($stmt->execute([$hashed_password, $_SESSION['sup_id']])) {
            $updateSuccess = "Password updated successfully! Please log in again.";
            session_destroy();
            header("Location: supporter_signin.php"); // Redirect to login page
            exit();
        } else {
